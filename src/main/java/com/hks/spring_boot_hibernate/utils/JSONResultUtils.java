package com.hks.spring_boot_hibernate.utils;
...
//    }
    ...
    public JSONResultUtils(Integer status, String msg, Object data) {
        this.status = status;
        this.msg = msg;
        this.data = data;
    }

    public JSONResultUtils(Object data) {
        this.status = 200;
        this.msg = "OK";
        this.data = data;
    }
     ...
     *
     */
    public static JSONResultUtils formatToPojo(String jsonData, Class<?> clazz) {
        try {
            if (clazz == null) {
                return MAPPER.readValue(jsonData, JSONResultUtils.class);
            }
     ...
     *
     */
    public static JSONResultUtils format(String json) {
        try {
            return MAPPER.readValue(json, JSONResultUtils.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
     ...
     *
     */
    public static JSONResultUtils formatToList(String jsonData, Class<?> clazz) {
        try {
            JsonNode jsonNode = MAPPER.readTree(jsonData);
            JsonNode data = jsonNode.get("data");
            Object obj = null;
